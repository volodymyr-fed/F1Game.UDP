...
sealed class AutoInlineArrayGenerator : IIncrementalGenerator
{
	...
	static void ExecuteGeneration(SourceProductionContext context, BuildTarget target)
	{
		...
		var typeNameEscaped = typeName.Replace('<', '{').Replace('>', '}');
		...
		string source = $$"""
			...
			/// <summary>
			/// Represents an inline array <see cref="{{typeNameEscaped}}"/> with {{length}} elements.
			...
			{{fieldGenerationCode}}
				...
				/// <summary>
				/// Returns a <see cref="Span{{{elementType}}}"/> that represents the elements of this inline array.
				/// </summary>
				/// <returns>
				/// A <see cref="Span{{{elementType}}}"/> of length <c>{{length}}</c> that provides mutable access to the elements of the inline array.
				/// </returns>
				public Span<{{elementType}}> AsSpan()
					=> MemoryMarshal.CreateSpan(ref Unsafe.As<{{typeName}}, {{elementType}}>(ref this), {{length}});

				/// <summary>
				/// Returns a <see cref="ReadOnlySpan{{{elementType}}}"/> that represents the elements of this inline array.
				/// </summary>
				/// <returns>
				/// A <see cref="ReadOnlySpan{{{elementType}}}"/> of length <c>{{length}}</c> that provides read-only access to the elements of the inline array.
				/// </returns>
				...
				/// <summary>
				/// Returns an <see cref="IEnumerable{{{elementType}}}"/> that enumerates the elements of this inline array.
				/// </summary>
				/// <returns>
				/// An <see cref="IEnumerable{{{elementType}}}"/> that iterates over the elements of the inline array in order.
				/// </returns>
				...
				/// <summary>
				/// Creates a new <see cref="{{typeNameEscaped}}"/> instance and populates it with elements from the specified <see cref="ReadOnlySpan{T}"/> source.
				/// </summary>
				/// <param name="source">
				...
				/// </param>
				/// <returns>
				/// A new <see cref="{{typeNameEscaped}}"/> instance containing elements from <paramref name="source"/>.
				/// </returns>
				...
				/// <summary>Determines whether two specified instances of <see cref="{{typeNameEscaped}}"/> are equal by comparing their elements sequence.</summary>
				...
				/// <summary>Determines whether two specified instances of <see cref="{{typeNameEscaped}}"/> are not equal by comparing their elements sequence.</summary>
				/// <param name="left">The first inline array to compare.</param>
				/// <param name="right">The second inline array to compare.</param>
				...
				/// <summary>
				/// Implicitly converts an array to a <see cref="{{typeNameEscaped}}"/>.
				...
				/// </summary>
				/// <param name="source">
				/// The source array to copy elements from. If <paramref name="source"/> is <c>null</c>, a default-initialized <see cref="{{typeNameEscaped}}"/> is returned.
				/// </param>
				/// <returns>
				/// A <see cref="{{typeNameEscaped}}"/> containing elements from <paramref name="source"/>.
				/// </returns>
				public static implicit operator {{typeName}}({{elementType}}[] source)
					=> source is null ? new() : Create(source);

				/// <summary>
				/// Implicitly converts a <see cref="ReadOnlySpan{{{elementType}}}"/> to a <see cref="{{typeNameEscaped}}"/>.
				...
				/// <summary>
				/// Implicitly converts a <see cref="Span{{{elementType}}}"/> to a <see cref="{{typeNameEscaped}}"/>.
				/// Copies up to <c>{{length}}</c> elements from the source span; extra elements are ignored, and missing elements are default-initialized.
				/// </summary>
				/// <param name="source">
				/// The source span to copy elements from.
